...
int	handle_redir(char **args, int *saved_stdout, int *saved_stdin)
{
	...
	*saved_stdin = dup(STDIN_FILENO);
	while (args[i])
	{
		...
		{
			fd = (ft_strcmp(args[i], ">") == 0) ? open(args[i + 1],
					O_CREAT | O_WRONLY | O_TRUNC, 0644) : open(args[i + 1],
			...
			close(fd);
			free(args[i]); // Libere o operador
			args[i] = NULL;
			free(args[i + 1]); // Libere o argumento associado
			args[i + 1] = NULL;
			i++;
		}
		...
		else if (!ft_strcmp(args[i], "<<"))
		{
			...
			free(args[i]); // Libere o operador
			args[i] = NULL;
			free(args[i + 1]); // Libere o delimitador
			args[i + 1] = NULL;
			i++;
		}
		i++;
